...
class QuestionsIndex extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let thisP = this.props.match.params;
    let nextP = nextProps.match.params;

    if (thisP.user_id !== nextP.user_id) {
      const params = {user: {
                              id: nextP.user_id
                            }
                      };
      ...
      this.props.fetchAllAnswers(params);
      this.setState({user: nextP.user_id});
    }
  ...
  render() {
    ...
    if ((this.props.answers.length > 0) &&
        (this.props.answers[0].user_id === user) &&
        (this.props.answeredQuestions.length > 0)) {

      let pairs = {};

      this.props.answeredQuestions.forEach(
        (question) => pairs[question.id] = [question]
      );
      ...
      answerItems = Object.keys(pairs).map(
        (item) => {
          return <AnswerItem
    ...
    if ((this.props.unansweredQuestions.length > 0) &&
        (user === this.props.currentUser.id)) {
          ...
          unansweredItems = this.props.unansweredQuestions.map(
                              (question) => {
